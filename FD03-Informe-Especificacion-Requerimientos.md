...
rectangle "Docente / Evaluador" as Docente
rectangle "Equipo de Desarrollo" as Equipo
rectangle "Frontend React" as Frontend
rectangle "Motores Simulados" as Motores
rectangle "Persistencia y Servicios" as Servicios
rectangle "Documentacion y Pruebas" as Docs
...
@startuml
start
:Necesidad de practicar bases de datos;
:Instalar motor requerido;
:Configurar servicio, puerto y credenciales;
:Instalar cliente o herramienta;
:Crear base y tablas;
:Cargar datos de prueba;
if (El entorno funciona?) then (Si)
  :Ejecutar consultas;
  :Capturar evidencias manualmente;
else (No)
  :Resolver errores de instalacion/configuracion;
...
@startuml
start
:Abrir Simulador de Bases de Datos;
:Iniciar sesion;
:Seleccionar motor;
:Importar datos o crear tablas;
:Escribir consulta en Monaco Editor;
if (Seleccion parcial?) then (Si)
  :Ejecutar solo seleccion;
else (No)
...
:Explorar esquema;
:Exportar evidencias;
if (Se requiere prueba de carga?) then (Si)
  :Configurar simulador de carga;
  :Ejecutar simulacion;
  :Guardar o exportar logs;
endif
:Finalizar practica;
stop
@enduml
...
:SQLEditor obtiene EngineTab activo;
:Determina query completa o seleccion;
:sqlEngine procesa segun motor;
if (Motor relacional?) then (Si)
  :AlaSQL ejecuta sentencia;
elseif (MongoDB?) then (Si)
  :executeMongoQuery evalua comando;
else (Redis)
  :executeRedisCommand evalua comando;
endif
:QueryResult generado;
:useStore guarda resultado;
:ResultsPanel renderiza tabla;
:queryLogger registra ejecucion;
stop
@enduml
```
...
@startuml
actor Usuario
participant "SQLEditor" as Editor
